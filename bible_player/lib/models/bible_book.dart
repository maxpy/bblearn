/// Data model for a Bible book.
///
/// Represents one of the 66 books of the Bible with bilingual names,
/// chapter count, and testament classification.
library;

/// Testament classification for Bible books.
enum Testament {
...
class BibleBook {
  ...
  /// Looks up a book by its canonical number (1-66).
  ///
  /// Returns `null` if [number] is out of range.
  static BibleBook? byNumber(int number) {
    if (number < 1 || number > 66) return null;
    return allBooks[number - 1];
  }

  /// Returns all Old Testament books.
  static List<BibleBook> get oldTestament =>
      allBooks.where((b) => b.testament == Testament.ot).toList();

  /// Returns all New Testament books.
  static List<BibleBook> get newTestament =>
      allBooks.where((b) => b.testament == Testament.nt).toList();

  @override
  String toString() => 'BibleBook($number: $nameEn / $nameZh)';
}
